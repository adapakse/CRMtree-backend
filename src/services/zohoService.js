...
// Returns the index in `text` where the quote section starts, or -1.
// Handles two citation formats that Zoho places in the same element as new content:
//   "On śr., 01 lip 2026 … wrote"  (Zoho inline, with or without leading dashes)
//   "śr., 1 lip 2026 o 17:22 … napisał(a):"  (Polish Gmail attribution at line end)
function findQuoteSepIndex(text) {
  const candidates = [];

  // Zoho "On [weekday]" citation — weekday-with-dot/comma makes it specific enough
  ...
  // Polish Gmail attribution — cut at the start of the line containing "napisał(a):"
  const napM = /napisał\(a\)\s*:/i.exec(text);
  if (napM && napM.index > 0) {
    const nlBefore = text.lastIndexOf('\n', napM.index - 1);
    // If no newline before it use the match index (attribution is on same line as new content)
...
// Never touches the phrase when it appears inside a longer sentence.
...
function stripZohoFooterElement($) {
  $('*').filter(function () {
...
// Strips Zoho-auto-appended quoted history from a reply body.
// Strategy: convert HTML to visible text, locate the earliest citation separator,
// return the text before it encoded as minimal safe HTML.
// Falls back to the cheerio-cleaned HTML when no separator is found.
function stripZohoQuotedContent(html) {
  if (!html) return html;

  // Plain-text body (no HTML tags)
  if (!html.includes('<')) {
    const withoutFooter = stripZohoFooterText(html);
    const sep = findQuoteSepIndex(withoutFooter);
    return sep > 0 ? withoutFooter.slice(0, sep).trimEnd() : withoutFooter;
  }

  const $ = cheerioLoad(html, { decodeEntities: false });
  // Remove standard Gmail quote containers first (blockquote, .gmail_quote, .gmail_attr)
  $('blockquote, .gmail_quote, .gmail_attr').remove();
  stripZohoFooterElement($);
  const cleaned = $('body').html() ?? '';

  // Convert to visible text and find separator
  const visText   = htmlToVisibleText(cleaned);
  const noFooter  = stripZohoFooterText(visText);
  const hasFooter = noFooter !== visText; // footer survived as inline text, not its own element
  const sep       = findQuoteSepIndex(noFooter);

  if (sep > 0 || hasFooter) {
    const before = (sep > 0 ? noFooter.slice(0, sep) : noFooter).trimEnd();
    if (!before) return cleaned; // nothing before separator — keep cheerio result
    // Re-encode as minimal safe HTML: entities + <br> for newlines
    return before
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
...
async function getEffectiveCreds(userId) {
  const { rows } = await pool.query(
    `SELECT u.tenant_id,
     ...
     LEFT JOIN tenant_email_providers ep
       ON ep.tenant_id = u.tenant_id AND ep.provider = 'zoho' AND ep.is_enabled = true
     WHERE u.id = $1`,
    [userId],
  );
  const row = rows[0];
  ...
  return {
    ...
    clientId:     hasDbCreds ? row.client_id           : config.zoho.clientId,
    clientSecret: hasDbCreds ? decrypt(row.ep_secret_enc) : config.zoho.clientSecret,
    redirectUri:  hasDbCreds ? (row.redirect_uri || config.zoho.redirectUri) : config.zoho.redirectUri,
  };
}
...
async function getThread(userId, threadId) {
  ...
  return withContent
    ...
    .map(m => {
      ...
      const cleaned     = stripZohoQuotedContent(fullContent);
