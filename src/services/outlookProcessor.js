"use strict";
// src/services/outlookProcessor.js
//
// Mirrors gmailProcessor.js's matching/dedup logic for Outlook. Outlook has
// no push mechanism (see outlookService.getNewMessages — delta query, no
// webhook), so there is no processNotification(emailAddress, historyId)
// entry point here — outlookPoller.js calls processUserNotifications(userId)
...
//
// Matches msg to a CRM lead or partner (by conversation → sender), dedupes,
// and records the activity / read record. Reuses the same
// crm_lead_activities/crm_partner_activities columns as Gmail
...
async function processOneMessage(msg, tenantId, userId, emailAddress) {
  console.log(`[OutlookProcessor] Przetwarzanie msg=${msg.id} threadId=${msg.threadId} from="${msg.from}" subject="${msg.subject}"`);
  ...
  console.log(`[OutlookProcessor] Dopasowanie wątku: leadId=${leadId} partnerId=${partnerId} tenant=${tenantId}`);
  ...
  if (!leadId && !partnerId) {
    ...
    if (!senderDomain || senderDomain === userDomain) {
      console.log(`[OutlookProcessor] Wewnętrzny email / brak domeny — pominięto`);
      return false;
    }
    ...
    if (!leadId && !partnerId) {
      console.log(`[OutlookProcessor] Nieznany nadawca ${senderEmail} — pominięto`);
      return false;
    }
    ...
    console.log(`[OutlookProcessor] Fallback match: ${actTable2} recordId=${recordId2} email="${senderEmail}" inserted=${insRes.rowCount} tenant=${tenantId}`);
    ...
    if (insRes.rowCount > 0) {
      ...
      for (const att of msg.attachments || []) {
        try {
          const buffer = await outlookService.getAttachmentBuffer(userId, msg.id, att.attachmentId);
          ...
          console.warn('[OutlookProcessor] Fallback attachment failed:', attErr.message);
        }
      }
    }
  ...
  try {
    ...
    console.warn(`[OutlookProcessor] crm_email_message_reads niedostępna (brak migracji?): ${dupCheckErr.message}`);
  }
  if (alreadyProcessed) {
    console.log(`[OutlookProcessor] Wiadomość ${msg.id} już przetworzona — pominięto`);
    return false;
  }
  ...
  console.log(`[OutlookProcessor] UPDATE aktywności wątku: ${updateRes.rowCount} wierszy (recordId=${recordId} tenant=${tenantId})`);
  ...
  try {
    ...
    console.log(`[OutlookProcessor] INSERT crm_email_message_reads: msgId=${msg.id} threadId=${msg.threadId}`);
  } catch (insertErr) {
    console.error(`[OutlookProcessor] INSERT crm_email_message_reads FAILED (msgId=${msg.id}): ${insertErr.message}`);
  }
  ...
  for (const att of msg.attachments || []) {
    try {
      ...
      console.warn("[OutlookProcessor] Attachment download failed:", attErr.message);
    }
  }
...
// ── Poll one connected Outlook mailbox for new mail ──────────────────────────
//
// Called by outlookPoller.js on each cycle for every row in
// user_outlook_tokens. Unlike Gmail's push-notification flow there is no
// historyId to persist here — outlookService.getNewMessages() already
// reads/writes delta_link on user_outlook_tokens itself.
//
async function processUserNotifications(userId) {
  const { rows } = await pool.query(
    `SELECT u.tenant_id, t.email
     FROM user_outlook_tokens t
  ...
  const { newMessages } = await outlookService.getNewMessages(userId);

  let processed = 0;
  for (const raw of newMessages) {
    if (raw["@removed"]) continue;
    try {
      const msg = await outlookService.getMessage(userId, raw.id);
      const ok  = await processOneMessage(msg, tenantId, userId, emailAddress);
      if (ok) processed++;
    } catch (msgErr) {
      console.error("[OutlookProcessor] Message processing error:", msgErr.message);
    }
  }
