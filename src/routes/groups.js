...
router.post('/',
  ...
  async (req, res, next) => {
    try {
      const { name, display_name, description, has_owner_restriction = false } = req.body;
      const { rows } = await db.query(
        `INSERT INTO group_profiles (name, display_name, description, has_owner_restriction, created_by)
         VALUES ($1,$2,$3,$4,$5)
         RETURNING *`,
        [name, display_name || name, description || null, has_owner_restriction, req.user.id]
      );
      await audit.log({
        user:       req.user,
...
router.patch('/:id',
  ...
  async (req, res, next) => {
    try {
      const { rows: before } = await db.query('SELECT * FROM group_profiles WHERE id = $1', [req.params.id]);
      if (!before.length) return res.status(404).json({ error: 'Group not found' });

      const allowed     = ['display_name', 'description', 'has_owner_restriction', 'is_active'];
      const setClauses  = [];
      const params      = [];
      let   p           = 1;
      ...
      const { rows } = await db.query(
        `UPDATE group_profiles SET ${setClauses.join(',')} WHERE id = $${p} RETURNING *`, params
      );
      await audit.log({
        user:        req.user,
...
// ────────────────────────────────────────────────────────────
router.delete('/:id', requireAdmin, [param('id').isUUID()], validate, async (req, res, next) => {
  try {
    const { rows: docCheck } = await db.query(
      'SELECT COUNT(*) FROM documents WHERE group_id = $1 AND deleted_at IS NULL', [req.params.id]
    );
    if (parseInt(docCheck.rows[0].count) > 0) {
      return res.status(409).json({
        error: 'Cannot delete group with active documents. Reassign documents first.',
      });
    }

    const { rows } = await db.query(
      'UPDATE group_profiles SET is_active = FALSE WHERE id = $1 RETURNING *', [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Group not found' });
