...
router.post('/',
  ...
  async (req, res, next) => {
    try {
      ...
      if (!userRows.length) return res.status(400).json({ error: 'Assignee not found' });
      ...
      await db.transaction(async (client) => {
        const { rows } = await client.query(
          `INSERT INTO workflow_tasks
           ...
           RETURNING *`,
          [doc.id, req.user.id, assigned_to, task_type, message || null, due_date || null]
        );
        taskRow = rows[0];
        ...
        await audit.log({
          user:      req.user, document: doc, action: 'workflow_task_created',
          afterState: { task_type, assigned_to, assignee_name: assignee.display_name, message },
...
router.patch('/:taskId',
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.body.message  !== undefined) { updates.push(`message = $${p++}`);  params.push(req.body.message); }
      if (req.body.due_date !== undefined) { updates.push(`due_date = $${p++}`); params.push(req.body.due_date || null); }
      ...
      const actionName = req.body.task_status === 'completed' ? 'workflow_task_completed'
                       : req.body.task_status === 'cancelled' ? 'workflow_task_cancelled'
                       : 'workflow_task_created';

      await audit.log({
        user:        req.user, document: doc, action: actionName,
...
// ────────────────────────────────────────────────────────────
router.delete('/:taskId', [param('taskId').isUUID()], validate, async (req, res, next) => {
  try {
    const doc = await loadDoc(req.params.documentId);
    if (!doc) return res.status(404).json({ error: 'Document not found' });
    await perms.assertCanFull(req.user.id, doc);
    ...
    await audit.log({
      user:        req.user, document: doc, action: 'workflow_task_cancelled',
