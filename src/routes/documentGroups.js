...
router.get('/:id', [param('id').isUUID()], validate, async (req, res, next) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ error: 'Document group not found' });
    res.json(rows[0]);
  } catch (err) { next(err); }
});
    ...
    try {
      ...
      await db.transaction(async (client) => {
        ...
        for (const docId of document_ids) {
          ...
          await audit.log({
            user:      req.user, action: 'document_linked',
        ...
        await audit.log({
          user:      req.user, action: 'doc_group_created',
          afterState: group,
...
router.patch('/:id',
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.body.name        !== undefined) { setClauses.push(`name = $${p++}`);        params.push(req.body.name); }
      if (req.body.description !== undefined) { setClauses.push(`description = $${p++}`); params.push(req.body.description || null); }

      if (!setClauses.length) return res.status(400).json({ error: 'No fields to update' });
      params.push(req.params.id);

      const { rows } = await db.query(
        `UPDATE document_groups SET ${setClauses.join(',')} WHERE id = $${p} RETURNING *`, params
      );
      if (!rows.length) return res.status(404).json({ error: 'Document group not found' });

      await audit.log({
        user:      req.user, action: 'doc_group_updated',
        afterState: req.body, metadata: { group_id: req.params.id },
        ipAddress: req.auditContext?.ipAddress,
      });
      res.json(rows[0]);
    } catch (err) { next(err); }
...
router.post('/:id/documents',
  ...
  async (req, res, next) => {
    try {
      const { rows: grp } = await db.query('SELECT id FROM document_groups WHERE id = $1', [req.params.id]);
      if (!grp.length) return res.status(404).json({ error: 'Document group not found' });
      ...
      await audit.log({
        user:      req.user, action: 'document_linked',
        metadata:  { document_group_id: req.params.id, document_id: req.body.document_id },
        ipAddress: req.auditContext?.ipAddress,
      });
...
router.delete('/:id/documents/:documentId',
  [param('id').isUUID(), param('documentId').isUUID()], validate,
  async (req, res, next) => {
    try {
      const { rows } = await db.query(
      ...
      if (!rows.length) return res.status(404).json({ error: 'Document not in this group' });

      await audit.log({
        user:      req.user, action: 'document_unlinked',
        metadata:  { document_group_id: req.params.id, document_id: req.params.documentId },
        ipAddress: req.auditContext?.ipAddress,
      });
...
router.delete('/:id', [param('id').isUUID()], validate, async (req, res, next) => {
  try {
    ...
    await db.query('UPDATE documents SET document_group_id = NULL WHERE document_group_id = $1', [req.params.id]);
    const { rows } = await db.query('DELETE FROM document_groups WHERE id = $1 RETURNING *', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Document group not found' });

    await audit.log({
      user:      req.user, action: 'doc_group_deleted',
      beforeState: rows[0], metadata: { group_id: req.params.id },
      ipAddress: req.auditContext?.ipAddress,
    });
