      ...
      await db.transaction(async (client) => {
        const { rows } = await client.query(
          `INSERT INTO documents
           ...
           RETURNING *`,
          [name, doc_type, gdpr_type, group_id, entities, ownerId,
           document_group_id || null, expiration_date || null,
           signing_date || null, req.user.id]
        );
        const doc = rows[0];
        ...
        if (req.file) {
          blobResult = await storage.uploadDocument(
            req.file.buffer, req.file.originalname, req.file.mimetype, doc.id, 1
          );
          await client.query(
        ...
        await audit.log({
          user: req.user, document: doc, action: 'document_created',
          afterState: { name, doc_type, gdpr_type, group_id, entities, owner_id: ownerId },
          ipAddress: req.auditContext?.ipAddress, userAgent: req.auditContext?.userAgent, client,
        });
...
router.get('/:id', [param('id').isUUID()], validate, async (req, res, next) => {
  try {
    ...
    const groupCanRead = req.user.is_admin || await perms.canRead(req.user.id, doc);
    let taskAccess = null;
    if (!groupCanRead) {
    ...
    await audit.log({
      user: req.user, document: doc, action: 'document_viewed',
      ipAddress: req.auditContext?.ipAddress, userAgent: req.auditContext?.userAgent,
    });
...
router.patch('/:id',
  ...
  async (req, res, next) => {
    try {
      ...
      const groupCanFull = req.user.is_admin || await perms.canFull(req.user.id, doc);
      if (!groupCanFull) {
      ...
      for (const field of allowed) {
        if (req.body[field] !== undefined) {
          ...
          if (field === 'doc_type')  setClauses.push(`doc_type = $${p++}::doc_type`);
          else if (field === 'gdpr_type') setClauses.push(`gdpr_type = $${p++}::gdpr_type`);
          else if (field === 'status')    setClauses.push(`status = $${p++}::doc_status`);
...
router.delete('/:id', [param('id').isUUID()], validate, async (req, res, next) => {
  try {
    ...
    await audit.log({
      user:        req.user, document: doc, action: 'document_deleted',
...
router.post('/:id/file',
  ...
  async (req, res, next) => {
    try {
      ...
      if (!groupCanFull) {
        const taskAccess = await getTaskAccess(req.user.id, doc.id);
        if (taskAccess !== 'full') return res.status(403).json({ error: 'Access denied' });
      }
      ...
      await db.transaction(async (client) => {
        await client.query(
          ...
          [doc.id, nextVer, req.body.label || `Version ${nextVer}`,
           blobResult.blobPath, blobResult.blobName, blobResult.blobSizeBytes, req.file.mimetype, req.user.id]
        );
        await client.query(
          `UPDATE documents SET blob_path=$1, blob_name=$2, blob_size_bytes=$3, mime_type=$4 WHERE id=$5`,
          [blobResult.blobPath, blobResult.blobName, blobResult.blobSizeBytes, req.file.mimetype, doc.id]
...
router.get('/:id/download', [param('id').isUUID()], validate, async (req, res, next) => {
  try {
    ...
    await audit.log({
      user: req.user, document: doc, action: 'document_downloaded',
      ipAddress: req.auditContext?.ipAddress, userAgent: req.auditContext?.userAgent,
    });
...
router.get('/:id/versions/:versionId/preview',
  ...
  async (req, res, next) => {
    try {
      ...
      const { buffer, contentType } = await storage.downloadDocument(verRows[0].blob_path);
      res.set({
        'Content-Type': contentType || 'application/pdf',
...
router.get('/:id/versions/:versionId/download',
  ...
  async (req, res, next) => {
    try {
      ...
      await audit.log({
        user: req.user, document: doc, action: 'document_downloaded',
        metadata: { version_id: verRows[0].id, version_number: verRows[0].version_number },
        ipAddress: req.auditContext?.ipAddress,
      });
      res.send(buffer);
    } catch (err) { next(err); }
